% single_fly_analysis_V18_one_file_only by Seung-Hye : for only one file (manually pick
% the file)

%% Big changes in V18
% 1. Velocity_Classifier is replaced by Velocity_Classifier1
% 2. stricter thresholds for stop: stopthreshold = .1 to .03, runthreshold
% = .16 to .1
% 3. curvature calculation is done with LineNormals2D instead of
% LineCurvature2D
% 4. two-step smoothing : 1st with 'sgolay'
%%
...
% This has been changed since V18!
fly_x = smooth(fly_x_original,10,'sgolay');
fly_y = smooth(fly_y_original,10,'sgolay');
%further smoothing
fly_x = smooth(fly_x,2);
fly_y = smooth(fly_y,2);
...
if isempty(analysis_option) == 1 % if user wants to continue with the other analyses
    ...
    turn_reorientation = cumsum(curv_long_CV_totalturn);
