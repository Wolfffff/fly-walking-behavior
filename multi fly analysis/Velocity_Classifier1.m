...
%now assign the value to the last two elements (instead of Nans)
if velocity_classified(end-2) == 0 %if it is stop
    velocity_classified(end-1:end) = 0;
    velocity_classified_binary(end-1:end) = 0;
else
    velocity_classified(end-1:end) = velocity(end-1:end);
    velocity_classified_binary(end-1:end) = 1;
end
