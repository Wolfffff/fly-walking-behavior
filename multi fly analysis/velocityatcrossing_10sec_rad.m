function [velocity_in2out_before,velocity_in2out_during,velocity_in2out_after,...
    ...
    crossing_out_during,crossing_out_after, vel_total,velocity_classified,rad_vel,rad_vel_sqrt, framespertimebin, timeperiods, timebefore, totaltime)
...
if isempty(crossing_in_before) == 1 %no crossing out 'before'
    period_1 = 2;
    display('There is no crossing in the before period');
    
elseif isempty(crossing_in_after) ==1 %no crossing out 'after'
    period_2 = 2;
...
radvel_out2in_ov = cell(3,1);
radvel_in2out_ov = cell(3,1);
...
for period = period_1:period_2 %each odor period (only the ones with at least one crossing event)
    if isempty(crossing_out_cell{period})==1
    ...
    if isempty(crossing_in_before) == 0
        if crossing_in_before(1) < timebefore*framespertimebin+1 %if there is less than timebefore sec before the first crossing in the before period
            ...
            runstoptoplot_out2in{1,1} = nan((crossing_out_cell{1,1}(1)-crossing_in_cell{1,1}(1)+timebefore*framespertimebin),1);
            ...
            %radial velocity : not sqrt, but original value (ov)
            radvel_out2in_ov{1,1} = nan((crossing_out_cell{1,1}(1)-crossing_in_cell{1,1}(1)+timebefore*framespertimebin),1);
            %then add the values
            radvel_out2in_ov{1,1}(((timebefore*framespertimebin)-crossing_in_before(1)+2):end) =...
                rad_vel(1:(crossing_out_cell{1}(1))-1);
            ...
            radvel_out2in_ov{1,1} =...
                rad_vel((crossing_in_cell{1}(1))-(timebefore*framespertimebin):(crossing_out_cell{1}(1))-1);
    ...
    if period > 1
        if isempty(crossing_out_cell{period-1}) == 0 %if there was any crossing in 'before'
            if (crossing_in_cell{period}(1)-(timebefore*framespertimebin)) < (crossing_out_cell{period-1}(end)); %if timebefore seconds before the crossing in is before the last crossing out
                ...
                %radial velocity calculation : original value (ov)
                radvel_out2in_ov{period, 1} = nan(maxrow+(timebefore*framespertimebin),1);
                radvel_out2in_ov{period, 1}(nannumber+1:nannumber+sizetoplot)...
                    = rad_vel((crossing_out_cell{period-1}(end)):crossing_out_cell{period}(1)-1); %just take the portion that is outside
                ...
                radvel_out2in_ov{period, 1} =...
                    rad_vel((crossing_in_cell{period}(1))-(timebefore*framespertimebin):(crossing_out_cell{period}(1)-1));
    ...
    for h = 2:length(crossing_in_cell{period});
        ...
        if crossing_in_cell{period}(h)-(timebefore*framespertimebin) < crossing_out_cell{period}(h-1); %if 2 seconds before the crossing in is before the last crossing out
            ...
            %rad vel (ov)
            radvel_out2in_ov{period, h} = nan(maxrow+(timebefore*framespertimebin),1);
            
            radvel_out2in_ov{period, h}(nannumber+1:nannumber+sizetoplot+1)...
                = rad_vel((crossing_out_cell{period}(h-1)):(crossing_out_cell{period}(h)-1)); %just take the portion that is outside
            ...
            radvel_out2in_ov{period, h} =...
                rad_vel((crossing_in_cell{period}(h))-(timebefore*framespertimebin):(crossing_out_cell{period}(h)-1));
    ...
    for h = 1:length(crossing_in_cell{period})-1;
        if crossing_out_cell{period}(h)-(timebefore*framespertimebin) < crossing_in_cell{period}(h); %if timebefore seconds before the crossing out is before the last crossing in
            ...
            %rad vel calculation: ov
            radvel_in2out_ov{period, h} = nan(maxrow+(timebefore*framespertimebin),1);
            radvel_in2out_ov{period, h}(nannumber+1:nannumber+sizetoplot+1)...
                = rad_vel((crossing_in_cell{period}(h)):(crossing_in_cell{period}(h+1)-1)); %just take the portion that is outside
            ...
            %rad vel calculation: ov
            radvel_in2out_ov{period, h} =...
                (rad_vel((crossing_out_cell{period}(h))-(timebefore*framespertimebin):((crossing_in_cell{period}(h+1)-1))));
    ...
    if notreal_out(period) == 0 %do the following only when last crossing_out is real crossing
        ...
        if crossing_out_cell{period}(end)-(timebefore*framespertimebin) < crossing_in_cell{period}(end); %there is less than timebefore sec before last crossing out and period end
            ...
            %rad vel calculation:ov
            radvel_in2out_ov{period, end+1} = nan(maxrow+(timebefore*framespertimebin),1);
            radvel_in2out_ov{period, end}(nannumber+1:nannumber+sizetoplot+1)...
                = rad_vel((crossing_in_cell{period}(end)):timeperiods(period+1)); %just take the portion that is outside
            ...
            %rad vel calculation:ov
            radvel_in2out_ov{period, end+1} =...
                (rad_vel((crossing_out_cell{period}(end))-(timebefore*framespertimebin):timeperiods(period+1)));
...
if period_2 == 2 %if there is no crossing in 'after' period
...
radvel_i2o_avg(:,2) = nanmean(radvel_in2out_during,2);
...
%%
%radial velocity original value array clean up and grouping
%deleting the empty elements in the cell array
%before
radvel_in2out_bf_ov = radvel_in2out_ov(1,:);
radvel_i2o_before_ov = radvel_in2out_bf_ov(~cellfun('isempty',radvel_in2out_bf_ov));

radvel_out2in_bf_ov = radvel_out2in_ov(1,:);
radvel_o2i_before_ov= radvel_out2in_bf_ov(~cellfun('isempty',radvel_out2in_bf_ov));

%during
radvel_in2out_dr_ov = radvel_in2out_ov(2,:);
radvel_i2o_during_ov = radvel_in2out_dr_ov(~cellfun('isempty',radvel_in2out_dr_ov));

radvel_out2in_dr_ov = radvel_out2in_ov(2,:);
radvel_o2i_during_ov = radvel_out2in_dr_ov(~cellfun('isempty',radvel_out2in_dr_ov));

%after
radvel_in2out_aft_ov = radvel_in2out_ov(3,:);
radvel_i2o_after_ov = radvel_in2out_aft_ov(~cellfun('isempty',radvel_in2out_aft_ov));

radvel_out2in_aft_ov = radvel_out2in_ov(3,:);
radvel_o2i_after_ov = radvel_out2in_aft_ov(~cellfun('isempty',radvel_out2in_aft_ov));


%copying everything to matrix (2 sec before crossing + 5 sec after
%crossing)
if period_1 == 2 %no crossing in the before period
    radvel_in2out_before_ov = nan(totaltime*framespertimebin,1);
else
    nb = length(radvel_i2o_before_ov);
    radvel_in2out_before_ov = nan(totaltime*framespertimebin,nb);
    for n=1:nb
        if length(radvel_i2o_before_ov{1,n}) >= totaltime*framespertimebin
            radvel_in2out_before_ov(1:totaltime*framespertimebin,n) = radvel_i2o_before_ov{1,n}(1:totaltime*framespertimebin,:);
        else
            radvel_in2out_before_ov(1:length(radvel_i2o_before_ov{1,n}),n) = radvel_i2o_before_ov{1,n};
        end
    end
end
radvel_i2o_ov_avg(:,1) = nanmean(radvel_in2out_before_ov,2);

nd = length(radvel_i2o_during_ov);
radvel_in2out_during_ov = nan(totaltime*framespertimebin,nd);
for n=1:nd
    if length(radvel_i2o_during_ov{1,n}) >= totaltime*framespertimebin
        radvel_in2out_during_ov(1:totaltime*framespertimebin,n) = radvel_i2o_during_ov{1,n}(1:totaltime*framespertimebin,:);
    else
        radvel_in2out_during_ov(1:length(radvel_i2o_during_ov{1,n}),n) =radvel_i2o_during_ov{1,n};
    end
end
radvel_i2o_ov_avg(:,2) = nanmean(radvel_in2out_during,2);

if period_2 == 2 %if there is no crossing in 'after' period
    radvel_in2out_after_ov = nan(totaltime*framespertimebin,1);
else
    na = length(radvel_i2o_after_ov);
    radvel_in2out_after_ov = nan(totaltime*framespertimebin,na);
    for n=1:na
        if length(radvel_i2o_after_ov{1,n}) >= totaltime*framespertimebin
            radvel_in2out_after_ov(1:totaltime*framespertimebin,n) = radvel_i2o_after_ov{1,n}(1:totaltime*framespertimebin,:);
        else
            radvel_in2out_after_ov(1:length(radvel_i2o_after_ov{1,n}),n) = radvel_i2o_after_ov{1,n};
        end
    end
end
radvel_i2o_ov_avg(:,3)= nanmean(radvel_in2out_after_ov,2);

%Out2in
if period_1 == 2 % no crossing in the before period
    radvel_out2in_before_ov = nan(totaltime*framespertimebin,1);
else
    nb = length(radvel_o2i_before);
    radvel_out2in_before_ov = nan(totaltime*framespertimebin,nb);
    for n=1:nb
        if length(vel_o2i_before{1,n}) >= totaltime*framespertimebin
            radvel_out2in_before_ov(1:totaltime*framespertimebin,n) = radvel_o2i_before_ov{1,n}(1:totaltime*framespertimebin,:);
        else
            radvel_out2in_before_ov(1:length(radvel_o2i_before_ov{1,n}),n) = radvel_o2i_before_ov{1,n};
        end
    end
end
radvel_o2i_ov_avg(:,1) = nanmean(radvel_out2in_before_ov,2);

nd = length(radvel_o2i_during_ov);
radvel_out2in_during_ov = nan(totaltime*framespertimebin,nd);
for n=1:nd
    if length(radvel_o2i_during_ov{1,n}) >= totaltime*framespertimebin
        radvel_out2in_during_ov(1:totaltime*framespertimebin,n) = radvel_o2i_during_ov{1,n}(1:totaltime*framespertimebin,:);
    else
        radvel_out2in_during_ov(1:length(radvel_o2i_during_ov{1,n}),n) = radvel_o2i_during_ov{1,n};
    end
end
radvel_o2i_ov_avg(:,2) = nanmean(radvel_out2in_during_ov,2);

if period_2 == 2 %no crossing in the 'after' period
    radvel_out2in_after_ov = nan(totaltime*framespertimebin,1);
else
    na = length(radvel_o2i_after_ov);
    radvel_out2in_after_ov = nan(totaltime*framespertimebin,na);
    for n=1:na
        if length(radvel_o2i_after_ov{1,n}) >= totaltime*framespertimebin
            radvel_out2in_after_ov(1:totaltime*framespertimebin,n) = radvel_o2i_after_ov{1,n}(1:totaltime*framespertimebin,:);
        else
            radvel_out2in_after_ov(1:length(radvel_o2i_after_ov{1,n}),n) = radvel_o2i_after_ov{1,n};
        end
    end
end
radvel_o2i_ov_avg(:,3) = nanmean(radvel_out2in_after_ov,2);
