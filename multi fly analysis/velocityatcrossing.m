function [velocity_in2out_before,velocity_in2out_during,velocity_in2out_after,...
    velocity_out2in_before,velocity_out2in_during,velocity_out2in_after, runstop_in2out_before,runstop_in2out_during,runstop_in2out_after,...
    ...
    crossing_out_during,crossing_out_after, fly_x,fly_y, framespertimebin,velocity_classified, timeperiods)
...
for period = periodNo_1:periodNo_2 %each odor period (only the ones with at least one crossing event)
    ...
    if period > 1
        ...
        else %if there was no crossing in 'before'
            velocitytoplot_out2in{period, 1} =...
                vel_total((crossing_in_cell{period}(1))-(2*framespertimebin):(crossing_out_cell{period}(1)-1));
            runstoptoplot_out2in{period, 1} =...
                velocity_classified((crossing_in_cell{period}(1))-(2*framespertimebin):(crossing_out_cell{period}(1)-1));
        ...
        if crossing_out_cell{period}(end)-(2*framespertimebin) < crossing_in_cell{period}(end); %there is less than 2 sec before last crossing out and period end
          ...
          runstoptoplot_in2out{period, end+1} = nan(maxrow+(2*framespertimebin),1);
            ...
            velocitytoplot_in2out{period, end+1} =...
                (vel_total(((crossing_out_cell{period}(end))-(2*framespertimebin)):timeperiods(period+1)-1));
       
            runstoptoplot_in2out{period, end+1} =(velocity_classified(((crossing_out_cell{period}(end))-(2*framespertimebin)):timeperiods(period+1)-1));
