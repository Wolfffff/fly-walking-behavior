function [velocity_in2out_before,velocity_in2out_during,velocity_in2out_after,...
    ...
    crossing_out_during,crossing_out_after, vel_total,rad_vel_sqrt, framespertimebin, timeperiods)
...
for period = periodNo_1:periodNo_2 %each odor period (only the ones with at least one crossing event)
    ...
    if isempty(crossing_in_before) == 0
        if crossing_in_before(1) < 10*framespertimebin+1 %if there is less than 10 sec before the first crossing in the before period
            %first pre-allocate the matrix so that the crossing frame is at
            %10 seconds
            velocitytoplot_out2in{1,1} = nan((crossing_out_cell{1,1}(1)-crossing_in_cell{1,1}(1)+10*framespertimebin),1);
            %then add the values
            velocitytoplot_out2in{1,1}(((10*framespertimebin)-crossing_in_before(1)+2):end) =...
            ...
            radvel_out2in{1,1} = nan((crossing_out_cell{1,1}(1)-crossing_in_cell{1,1}(1)+10*framespertimebin),1);
            %then add the values
            radvel_out2in{1,1}(((10*framespertimebin)-crossing_in_before(1)+2):end) =...
            ...
            velocitytoplot_out2in{1,1} =...
                vel_total((crossing_in_cell{1}(1))-(10*framespertimebin):(crossing_out_cell{1}(1))-1);
            
            radvel_out2in{1,1} =...
                rad_vel_sqrt((crossing_in_cell{1}(1))-(10*framespertimebin):(crossing_out_cell{1}(1))-1);
 
            %save which frames I used to get the velocity
            crossing_o2i_frames{1,1} = (crossing_in_cell{1}(1))-(10*framespertimebin):(crossing_out_cell{1}(1))-1;
    ...
    if period > 1
        if isempty(crossing_out_cell{period-1}) == 0 %if there was any crossing in 'before'
            if (crossing_in_cell{period}(1)-(10*framespertimebin)) < (crossing_out_cell{period-1}(end)); %if 10 seconds before the crossing in is before the last crossing out
                maxrow = max(diff(crossing_in_cell{period}));
                velocitytoplot_out2in{period, 1} = nan(maxrow+(10*framespertimebin),1);
                nannumber = 10*framespertimebin- (crossing_in_cell{period}(1)-(crossing_out_cell{period-1}(end)));%how many nans will it take to make this vector the same length?
                ...
                %radial velocity calculation
                radvel_out2in{period, 1} = nan(maxrow+(10*framespertimebin),1);
                ...
                velocitytoplot_out2in{period, 1} =...
                    vel_total((crossing_in_cell{period}(1))-(10*framespertimebin):(crossing_out_cell{period}(1)-1));
                %radial velocity calculation
                radvel_out2in{period, 1} =...
                    rad_vel_sqrt((crossing_in_cell{period}(1))-(10*framespertimebin):(crossing_out_cell{period}(1)-1));
                
                %save which frames I used to get the velocity
                crossing_o2i_frames{period,1} = (crossing_in_cell{period}(1))-(10*framespertimebin):(crossing_out_cell{period}(1)-1);
    ...
    for h = 2:length(crossing_in_cell{period});
        ...
        if crossing_in_cell{period}(h)-(10*framespertimebin) < crossing_out_cell{period}(h-1); %if 2 seconds before the crossing in is before the last crossing out
            maxrow = max(diff(crossing_in_cell{period}));
            velocitytoplot_out2in{period, h} = nan(maxrow+(10*framespertimebin),1);
            nannumber = 10*framespertimebin- (crossing_in_cell{period}(h)-(crossing_out_cell{period}(h-1)));%how many nans will it take to make this vector the same length?
            ...
            %rad vel calculation
            radvel_out2in{period, h} = nan(maxrow+(10*framespertimebin),1);
            ...
            velocitytoplot_out2in{period, h} =...
                vel_total((crossing_in_cell{period}(h))-(10*framespertimebin):(crossing_out_cell{period}(h)-1));
            %rad vel calculation
            radvel_out2in{period, h} =...
                rad_vel_sqrt((crossing_in_cell{period}(h))-(10*framespertimebin):(crossing_out_cell{period}(h)-1));
            
            %save which frames I used to get the velocity
            crossing_o2i_frames{period,h} = (crossing_in_cell{period}(h))-(10*framespertimebin):(crossing_out_cell{period}(h)-1);
    ...
    for h = 1:length(crossing_in_cell{period})-1;
        if crossing_out_cell{period}(h)-(10*framespertimebin) < crossing_in_cell{period}(h); %if 2 seconds before the crossing out is before the last crossing in
            maxrow = max(diff(crossing_out_cell{period}));
            velocitytoplot_in2out{period, h} = nan(maxrow+(10*framespertimebin),1);
            nannumber = 10*framespertimebin- (crossing_out_cell{period}(h)-(crossing_in_cell{period}(h)));%how many nans will it take to make this vector the same length?
            ...
            %rad vel calculation
            radvel_in2out{period, h} = nan(maxrow+(10*framespertimebin),1);
            ...
            velocitytoplot_in2out{period, h} =...
                vel_total((crossing_out_cell{period}(h))-(10*framespertimebin):((crossing_in_cell{period}(h+1)-1)));
            %rad vel calculation
            radvel_in2out{period, h} =...
                (rad_vel_sqrt((crossing_out_cell{period}(h))-(10*framespertimebin):((crossing_in_cell{period}(h+1)-1))));
            %save which frames I used to get the velocity
            crossing_i2o_frames{period,h} = (crossing_out_cell{period}(h))-(10*framespertimebin):((crossing_in_cell{period}(h+1)-1));
    ...
    if notreal_out(period) == 0 %do the following only when last crossing_out is real crossing
        ...
        if crossing_out_cell{period}(end)-(10*framespertimebin) < crossing_in_cell{period}(end); %there is less than 2 sec before last crossing out and period end
            maxrow = max(diff(crossing_out_cell{period}));
            velocitytoplot_in2out{period, end+1} = nan(maxrow+(10*framespertimebin),1);
            nannumber = 10*framespertimebin- (crossing_out_cell{period}(end)-crossing_in_cell{period}(end));%how many nans will it take to make this vector the same length?
            ...
            %rad vel calculation
            radvel_in2out{period, end+1} = nan(maxrow+(10*framespertimebin),1);
            ...
            velocitytoplot_in2out{period, end+1} =...
                (vel_total((crossing_out_cell{period}(end))-(10*framespertimebin):timeperiods(period+1)));
            %rad vel calculation
            radvel_in2out{period, end+1} =...
                (rad_vel_sqrt((crossing_out_cell{period}(end))-(10*framespertimebin):timeperiods(period+1)));
            %save which frames I used to get the velocity
            crossing_i2o_frames{period,end+1}=(crossing_out_cell{period}(end))-(10*framespertimebin):timeperiods(period+1);
...
%copying everything to matrix (10 sec before crossing + 10 sec after
%crossing)
if periodNo_1 == 2 %no crossing in the before period
    velocity_in2out_before = nan(20*framespertimebin,1);
else
    nb = length(vel_i2o_before);
    velocity_in2out_before = nan(20*framespertimebin,nb);
    for n=1:nb
        if length(vel_i2o_before{1,n}) >= 20*framespertimebin
            velocity_in2out_before(1:20*framespertimebin,n) = vel_i2o_before{1,n}(1:20*framespertimebin,:);
        else
...
velocity_in2out_during = nan(20*framespertimebin,nd);
for n=1:nd
    if length(vel_i2o_during{1,n}) >= 20*framespertimebin
        velocity_in2out_during(1:20*framespertimebin,n) = vel_i2o_during{1,n}(1:20*framespertimebin,:);
    else
...
if periodNo_2 == 2 %if there is no crossing in 'after' period
    velocity_in2out_after = nan(20*framespertimebin,1);
else
    na = length(vel_i2o_after);
    velocity_in2out_after = nan(20*framespertimebin,na);
    for n=1:na
        if length(vel_i2o_after{1,n}) >= 20*framespertimebin
            velocity_in2out_after(1:20*framespertimebin,n) = vel_i2o_after{1,n}(1:20*framespertimebin,:);
        else
...
%Out2in
if periodNo_1 == 2 % no crossing in the before period
    velocity_out2in_before = nan(20*framespertimebin,1);
else
    nb = length(vel_o2i_before);
    velocity_out2in_before = nan(20*framespertimebin,nb);
    for n=1:nb
        if length(vel_o2i_before{1,n}) >= 20*framespertimebin
            velocity_out2in_before(1:20*framespertimebin,n) = vel_o2i_before{1,n}(1:20*framespertimebin,:);
        else
...
velocity_out2in_during = nan(20*framespertimebin,nd);
for n=1:nd
    if length(vel_o2i_during{1,n}) >= 20*framespertimebin
        velocity_out2in_during(1:20*framespertimebin,n) = vel_o2i_during{1,n}(1:20*framespertimebin,:);
    else
...
if periodNo_2 == 2 %no crossing in the 'after' period
    velocity_out2in_after = nan(20*framespertimebin,1);
else
    na = length(vel_o2i_after);
    velocity_out2in_after = nan(20*framespertimebin,na);
    for n=1:na
        if length(vel_o2i_after{1,n}) >= 20*framespertimebin
            velocity_out2in_after(1:20*framespertimebin,n) = vel_o2i_after{1,n}(1:20*framespertimebin,:);
        else
...
%crossing)
if periodNo_1 == 2 %no crossing in the before period
    radvel_in2out_before = nan(20*framespertimebin,1);
else
    nb = length(radvel_i2o_before);
    radvel_in2out_before = nan(20*framespertimebin,nb);
    for n=1:nb
        if length(radvel_i2o_before{1,n}) >= 20*framespertimebin
            radvel_in2out_before(1:20*framespertimebin,n) = radvel_i2o_before{1,n}(1:20*framespertimebin,:);
        else
...
radvel_in2out_during = nan(20*framespertimebin,nd);
for n=1:nd
    if length(radvel_i2o_during{1,n}) >= 20*framespertimebin
        radvel_in2out_during(1:20*framespertimebin,n) = radvel_i2o_during{1,n}(1:20*framespertimebin,:);
    else
...
if periodNo_2 == 2 %if there is no crossing in 'after' period
    radvel_in2out_after = nan(20*framespertimebin,1);
else
    na = length(radvel_i2o_after);
    radvel_in2out_after = nan(20*framespertimebin,na);
    for n=1:na
        if length(radvel_i2o_after{1,n}) >= 20*framespertimebin
            radvel_in2out_after(1:20*framespertimebin,n) = radvel_i2o_after{1,n}(1:20*framespertimebin,:);
        else
...
%Out2in
if periodNo_1 == 2 % no crossing in the before period
    radvel_out2in_before = nan(20*framespertimebin,1);
else
    nb = length(radvel_o2i_before);
    radvel_out2in_before = nan(20*framespertimebin,nb);
    for n=1:nb
        if length(vel_o2i_before{1,n}) >= 20*framespertimebin
            radvel_out2in_before(1:20*framespertimebin,n) = radvel_o2i_before{1,n}(1:20*framespertimebin,:);
        else
...
radvel_out2in_during = nan(20*framespertimebin,nd);
for n=1:nd
    if length(radvel_o2i_during{1,n}) >= 20*framespertimebin
        radvel_out2in_during(1:20*framespertimebin,n) = radvel_o2i_during{1,n}(1:20*framespertimebin,:);
    else
...
if periodNo_2 == 2 %no crossing in the 'after' period
    radvel_out2in_after = nan(20*framespertimebin,1);
else
    na = length(vel_o2i_after);
    radvel_out2in_after = nan(20*framespertimebin,na);
    for n=1:na
        if length(radvel_o2i_after{1,n}) >= 20*framespertimebin
            radvel_out2in_after(1:20*framespertimebin,n) = radvel_o2i_after{1,n}(1:20*framespertimebin,:);
        else
            radvel_out2in_after(1:length(radvel_o2i_after{1,n}),n) = radvel_o2i_after{1,n};
        end
    end
end
radvel_o2i_avg(:,3) = nanmean(radvel_out2in_after,2);
